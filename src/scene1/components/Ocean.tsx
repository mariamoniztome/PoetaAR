...
const vertexShader = `
  ...
  void main() {
    ...
    float elevation = cnoise(vec3(modelPosition.xz * 0.2, uTime * 0.2)) * (0.5 + uEnergy * 1.5);
    elevation += cnoise(vec3(modelPosition.xz * 0.8, uTime * 0.5)) * (0.1 + uEnergy * 0.5);
...
export function Ocean() {
  ...
  useFrame((state) => {
    if (materialRef.current) {
      ...
      materialRef.current.uniforms.uEnergy.value = useStore.getState().energy;
    }
  });
